...
void main()
{
    ...
    if (nMatch == -1)
    {
		...
		int iWhisper	= GetLocalInt(OBJECT_SELF, "npc_whisper");
		int iDistance	= GetLocalInt(OBJECT_SELF, "npc_distance_ignore");
		int iCutscene	= GetLocalInt(OBJECT_SELF, "npc_cutscene_disable");
		int iGreeting;
		
		if (GetLocalInt(OBJECT_SELF, "npc_no_greeting"))
		{
			iGreeting = FALSE;
		}
		else
        ...
        if (GetCommandable(OBJECT_SELF))
        {
			...
			ActionStartConversation(oShouter, "", iWhisper, iGreeting, iDistance, iCutscene);
        }
        else
        // * July 31 2004
